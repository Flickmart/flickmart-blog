"use client";

import {
  CalendarIcon,
  ChevronsUpDown,
  HomeIcon,
  List,
  MailIcon,
  PencilIcon,
  Sun,
} from "lucide-react";
import { AnimatePresence, motion } from "motion/react";
import Link from "next/link";
import { usePathname } from "next/navigation";
import { useTheme } from "next-themes";
import type React from "react";
import { useEffect, useRef, useState } from "react";
import { TableOfContents } from "@/components/table-of-contents";
import { Button, buttonVariants } from "@/components/ui/button";
import { Dock, DockIcon } from "@/components/ui/dock";
import { Separator } from "@/components/ui/separator";
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
  TooltipTrigger,
} from "@/components/ui/tooltip";
import { useTOC } from "@/contexts/toc-context";
import { useScrollDirection } from "@/hooks/use-scroll-direction";
import { cn } from "@/lib/utils";

export type IconProps = React.HTMLAttributes<SVGElement>;

const Icons = {
  calendar: (props: IconProps) => <CalendarIcon {...props} />,
  email: (props: IconProps) => <MailIcon {...props} />,
  toc: (props: IconProps) => <List {...props} />,
  x: (props: IconProps) => (
    <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" {...props}>
      <title>X</title>
      <path
        d="M18.901 1.153h3.68l-8.04 9.19L24 22.846h-7.406l-5.8-7.584-6.638 7.584H.474l8.6-9.83L0 1.154h7.594l5.243 6.932ZM17.61 20.644h2.039L6.486 3.24H4.298Z"
        fill="currentColor"
      />
    </svg>
  ),
  youtube: (props: IconProps) => (
    <svg
      fill="currentColor"
      height="32px"
      viewBox="0 0 32 32"
      width="32px"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <title>youtube</title>
      <path d="M29.41,9.26a3.5,3.5,0,0,0-2.47-2.47C24.76,6.2,16,6.2,16,6.2s-8.76,0-10.94.59A3.5,3.5,0,0,0,2.59,9.26,36.13,36.13,0,0,0,2,16a36.13,36.13,0,0,0,.59,6.74,3.5,3.5,0,0,0,2.47,2.47C7.24,25.8,16,25.8,16,25.8s8.76,0,10.94-.59a3.5,3.5,0,0,0,2.47-2.47A36.13,36.13,0,0,0,30,16,36.13,36.13,0,0,0,29.41,9.26ZM13.2,20.2V11.8L20.47,16Z" />
    </svg>
  ),
  github: (props: IconProps) => (
    <svg viewBox="0 0 438.549 438.549" {...props}>
      <title>GitHub</title>
      <path
        d="M409.132 114.573c-19.608-33.596-46.205-60.194-79.798-79.8-33.598-19.607-70.277-29.408-110.063-29.408-39.781 0-76.472 9.804-110.063 29.408-33.596 19.605-60.192 46.204-79.8 79.8C9.803 148.168 0 184.854 0 224.63c0 47.78 13.94 90.745 41.827 128.906 27.884 38.164 63.906 64.572 108.063 79.227 5.14.954 8.945.283 11.419-1.996 2.475-2.282 3.711-5.14 3.711-8.562 0-.571-.049-5.708-.144-15.417a2549.81 2549.81 0 01-.144-25.406l-6.567 1.136c-4.187.767-9.469 1.092-15.846 1-6.374-.089-12.991-.757-19.842-1.999-6.854-1.231-13.229-4.086-19.13-8.559-5.898-4.473-10.085-10.328-12.56-17.556l-2.855-6.57c-1.903-4.374-4.899-9.233-8.992-14.559-4.093-5.331-8.232-8.945-12.419-10.848l-1.999-1.431c-1.332-.951-2.568-2.098-3.711-3.429-1.142-1.331-1.997-2.663-2.568-3.997-.572-1.335-.098-2.43 1.427-3.289 1.525-.859 4.281-1.276 8.28-1.276l5.708.853c3.807.763 8.516 3.042 14.133 6.851 5.614 3.806 10.229 8.754 13.846 14.842 4.38 7.806 9.657 13.754 15.846 17.847 6.184 4.093 12.419 6.136 18.699 6.136 6.28 0 11.704-.476 16.274-1.423 4.565-.952 8.848-2.383 12.847-4.285 1.713-12.758 6.377-22.559 13.988-29.41-10.848-1.14-20.601-2.857-29.264-5.14-8.658-2.286-17.605-5.996-26.835-11.14-9.235-5.137-16.896-11.516-22.985-19.126-6.09-7.614-11.088-17.61-14.987-29.979-3.901-12.374-5.852-26.648-5.852-42.826 0-23.035 7.52-42.637 22.557-58.817-7.044-17.318-6.379-36.732 1.997-58.24 5.52-1.715 13.706-.428 24.554 3.853 10.85 4.283 18.794 7.952 23.84 10.994 5.046 3.041 9.089 5.618 12.135 7.708 17.705-4.947 35.976-7.421 54.818-7.421s37.117 2.474 54.823 7.421l10.849-6.849c7.419-4.57 16.18-8.758 26.262-12.565 10.088-3.805 17.802-4.853 23.134-3.138 8.562 21.509 9.325 40.922 2.279 58.24 15.036 16.18 22.559 35.787 22.559 58.817 0 16.178-1.958 30.497-5.853 42.966-3.9 12.471-8.941 22.457-15.125 29.979-6.191 7.521-13.901 13.85-23.131 18.986-9.232 5.14-18.182 8.85-26.84 11.136-8.662 2.286-18.415 4.004-29.263 5.146 9.894 8.562 14.842 22.077 14.842 40.539v60.237c0 3.422 1.19 6.279 3.572 8.562 2.379 2.279 6.136 2.95 11.276 1.995 44.163-14.653 80.185-41.062 108.068-79.226 27.88-38.161 41.825-81.126 41.825-128.906-.01-39.771-9.818-76.454-29.414-110.049z"
        fill="currentColor"
      />
    </svg>
  ),
};

const DATA = {
  navbar: [
    { href: "/", icon: HomeIcon, label: "Home" },
    { href: "/blog", icon: PencilIcon, label: "Blog" },
  ],
  contact: {
    social: {
      GitHub: {
        name: "GitHub",
        url: "https://github.com/Joseph-ebuka",
        icon: Icons.github,
      },
      X: {
        name: "X",
        url: "https://x.com/EbukaJoseph_dev",
        icon: Icons.x,
      },
      email: {
        name: "Send Email",
        url: "mailto:ebukaj665@gmail.com",
        icon: Icons.email,
      },
    },
  },
};

export function AppDock() {
  const scrollDirection = useScrollDirection();
  const pathname = usePathname();
  const { theme, setTheme } = useTheme();
  const { headings: tocHeadings } = useTOC();
  const [showTOC, setShowTOC] = useState(false);
  const [dockWidth, setDockWidth] = useState(0);
  const tocRef = useRef<HTMLDivElement>(null);
  const dockRef = useRef<HTMLDivElement>(null);

  const toggleTheme = () => {
    setTheme(theme === "light" ? "dark" : "light");
  };

  const toggleTOC = () => {
    setShowTOC(!showTOC);
  };

  // Handle click outside to close TOC
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        showTOC &&
        tocRef.current &&
        dockRef.current &&
        !tocRef.current.contains(event.target as Node) &&
        !dockRef.current.contains(event.target as Node)
      ) {
        setShowTOC(false);
      }
    };

    if (showTOC) {
      document.addEventListener("mousedown", handleClickOutside);
    }

    return () => {
      document.removeEventListener("mousedown", handleClickOutside);
    };
  }, [showTOC]);

  // Measure dock width and update TOC width to match
  useEffect(() => {
    const updateDockWidth = () => {
      if (dockRef.current) {
        const width = dockRef.current.offsetWidth;
        setDockWidth(width);
      }
    };

    // Initial measurement
    updateDockWidth();

    // Update on resize
    window.addEventListener("resize", updateDockWidth);

    // Use ResizeObserver to detect dock size changes (including hover expansion)
    let resizeObserver: ResizeObserver;
    if (dockRef.current) {
      resizeObserver = new ResizeObserver(updateDockWidth);
      resizeObserver.observe(dockRef.current);
    }

    return () => {
      window.removeEventListener("resize", updateDockWidth);
      if (resizeObserver) {
        resizeObserver.disconnect();
      }
    };
  }, []);

  // Only show TOC icon on blog post pages that have headings
  const isBlogPost = pathname.startsWith("/blog/") && pathname !== "/blog";
  const showTOCIcon = isBlogPost && tocHeadings.length > 0;

  return (
    <motion.div
      animate={{
        y: scrollDirection === "down" && !showTOC ? 100 : 0,
        opacity: scrollDirection === "down" && !showTOC ? 0 : 1,
      }}
      className="fixed right-0 bottom-4 left-0 z-[1000] flex justify-center"
      initial={{ y: 0, opacity: 1 }}
      transition={{
        duration: 0.3,
        ease: "easeInOut",
      }}
    >
      <div className="flex flex-col items-center justify-center">
        {/* TOC Panel */}
        <AnimatePresence>
          {showTOC && tocHeadings.length > 0 && (
            <motion.div
              animate={{ opacity: 1, y: 0, scale: 1 }}
              className="overflow-hidden rounded-t-3xl border border-b-0 bg-background/90 p-6 backdrop-blur-md border-border"
              exit={{ opacity: 0, y: 20, scale: 0.95 }}
              initial={{ opacity: 0, y: 20, scale: 0.95 }}
              ref={tocRef}
              style={{
                width: dockWidth > 0 ? `${dockWidth}px` : "auto",
                minWidth: dockWidth > 0 ? `${dockWidth}px` : "auto",
                height: "300px",
              }}
              transition={{ duration: 0.2, ease: "easeOut" }}
            >
              <div className="h-full overflow-y-auto">
                <TableOfContents headings={tocHeadings} />
              </div>
            </motion.div>
          )}
        </AnimatePresence>

        <TooltipProvider>
          <Dock
            className={cn(
              "mt-0 gap-1 bg-background/90 backdrop-blur-md border border-border",
              showTOC
                ? "rounded-t-none rounded-b-2xl border-t-0"
                : "rounded-3xl",
            )}
            direction="middle"
            ref={dockRef}
          >
            {showTOCIcon && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    className={cn(
                      "h-12 rounded-full bg-brand-primary/10 px-4 py-2 text-brand-primary text-sm hover:bg-brand-primary/20 border border-brand-primary/20",
                    )}
                    onClick={toggleTOC}
                    variant="ghost"
                  >
                    On this page <ChevronsUpDown className="size-2" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Table of Contents</p>
                </TooltipContent>
              </Tooltip>
            )}
            {DATA.navbar.map((item) => {
              const isActive =
                item.href === "/"
                  ? pathname === "/"
                  : pathname.startsWith(item.href);
              return (
                <DockIcon key={item.label}>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <div className="relative">
                        <Link
                          aria-label={item.label}
                          className={cn(
                            buttonVariants({ variant: "ghost", size: "icon" }),
                            "rounded-full md:size-12",
                            isActive && "bg-accent",
                          )}
                          href={item.href}
                        >
                          <item.icon className="size-4" />
                        </Link>
                      </div>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>{item.label}</p>
                    </TooltipContent>
                  </Tooltip>
                </DockIcon>
              );
            })}
            <Separator
              className="hidden h-full md:block"
              orientation="vertical"
            />
            {Object.entries(DATA.contact.social).map(([name, social]) => {
              // Show only GitHub and X (Twitter) on mobile
              const showOnMobile = name === "X";
              return (
                <DockIcon
                  className={showOnMobile ? "flex" : "hidden md:flex"}
                  key={name}
                >
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Link
                        aria-label={social.name}
                        className={cn(
                          buttonVariants({ variant: "ghost", size: "icon" }),
                          "size-6 rounded-full md:size-12",
                        )}
                        href={social.url}
                      >
                        <social.icon className="size-4" />
                      </Link>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>{name}</p>
                    </TooltipContent>
                  </Tooltip>
                </DockIcon>
              );
            })}

            <Separator
              className="h-full rounded-full py-2"
              orientation="vertical"
            />
            <DockIcon>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    className="size-6 rounded-full md:size-12"
                    onClick={toggleTheme}
                    size="icon"
                    variant="ghost"
                  >
                    <Sun className="size-4" />
                  </Button>
                </TooltipTrigger>
                <TooltipContent>
                  <p>Theme</p>
                </TooltipContent>
              </Tooltip>
            </DockIcon>
          </Dock>
        </TooltipProvider>
      </div>
    </motion.div>
  );
}
